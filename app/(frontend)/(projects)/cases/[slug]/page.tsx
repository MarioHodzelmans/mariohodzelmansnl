  ...
  return {
    ...
    openGraph: {
      ...
      images: item.featuredImage
        ? [{ url: item.featuredImage, alt: item.title }]
        : undefined,
    },
  };
}
      ...
      <section className="mxd-section">
        <div className="mxd-container grid-l-container">
          ...
          <div className="mxd-block">
            ...
            <div className="mb-4">
              {item.category ? (
                <span className="tag tag-m meta-tag">{item.category}</span>
              ) : null}
              {item.year ? (
                <span className="tag tag-m meta-tag">{item.year}</span>
              ) : null}
              {item.services.map((service) => (
                <span key={service} className="tag tag-m meta-tag">
                  {service}
                </span>
              ))}
            </div>
